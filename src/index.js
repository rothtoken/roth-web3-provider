...
class rothWeb3Provider extends EventEmitter {
  constructor(config) {
    super();
  ...
  request(payload) {
    ...
    var that = this;
    if (!(this instanceof rothWeb3Provider)) {
    ...
    switch (payload.method) {
      ...
      default:
        throw new ProviderRpcError(
          4200,
          `roth does not support calling ${payload.method} synchronously without a callback. Please provide a callback parameter to call ${payload.method} asynchronously.`
        );
    }
    return response;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      switch (payload.method) {
        ...
        case "eth_subscribe":
          throw new ProviderRpcError(
            4200,
            `roth does not support calling ${payload.method}. Please use your own solution`
          );
        default:
          // call upstream rpc
  ...
  postMessage(handler, id, data) {
    if (this.ready || handler === "requestAccounts") {
      ...
      if (window.rothwallet.postMessage) {
        window.rothwallet.postMessage(object);
      } else {
        // old clients
